class VolumeSliding {
  ...
  handleSliding = (e) => {
    ...
    requestAnimationFrame(() => {
      const rightPosition = this.volumeBar.offsetLeft + this.volumeBar.offsetWidth;

      const leftPosition = this.volumeBar.offsetLeft;

      const range = Math.max(0, Math.min(rightPosition - e.clientX, rightPosition - leftPosition));

      this.volumeThumb.style = `transform: translate(${-range}px, -50%); transition: none`;

      this.volumeFill.style = `max-width: ${this.volumeBar.offsetWidth - range}px; transition: none`;
  ...
  executeSlidingLogic() {
    ...
    document.querySelector("#volume-btn").onclick = () => {
      if (this.volumeIcon.classList.contains("fa-volume-down")) {
        ...
        const range = this.volumeBar.offsetLeft + this.volumeBar.offsetWidth - this.volumeBar.offsetLeft;
        this.volumeThumb.style = `transform: translate(${-range}px, -50%); transition: none`;
        this.volumeIcon.classList.replace("fa-volume-down", "fa-volume-xmark");
      } else if (this.volumeIcon.classList.contains("fa-volume-xmark")) {
        this.audio.volume = 1;
        this.volumeFill.style.maxWidth = `${
          this.volumeBar.offsetLeft + this.volumeBar.offsetWidth - this.volumeBar.offsetLeft
        }px`;
